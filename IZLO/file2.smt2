...
(define-fun y_prime ((D_prime Int) (E_prime Int)) Int
  (ite (< (+ x E_prime) (* 2 D_prime))
       (+ x (* 3 B))
       (* x (* 2 C))
  )
)

(define-fun z_prime ((D_prime Int) (E_prime Int)) Int
  (ite (<= (- (y_prime D_prime E_prime) 5) C)
       (- (* x A) (* (y_prime D_prime E_prime) B))
       (ite (> (+ x 2) D_prime)
            (* (+ x A) (+ (y_prime D_prime E_prime) B))
            (+ (* x B) (* (y_prime D_prime E_prime) A))
       )
  )
)
 ; if (z_3 <= D_3 + E_2) {
...
(assert
  (not (exists ((D_prime Int) (E_prime Int))
         (and 
           (> D_prime 0)
           (> E_prime 0)
           (> (z_prime D_prime E_prime) (+ D_prime E_prime))
           (< (+ D_prime E_prime) sum)
         )
       )
  )
