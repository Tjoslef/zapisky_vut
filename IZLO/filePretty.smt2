...
;f= true;
 ; }       
(define-fun f ((D Int) (E Int) (z Int)) Bool
  (and (> D 0) (> E 0) (> z (+ D E)))
)
(assert (f D E z))
; Pro všechny neexistují D' a E', které jsou pravdivé a jejich součet je menší než D + E
(assert 
  (not (exists ((D_prime Int) (E_prime Int))      
      ...
      (let ((y_prime (ite (< (+ x E_prime) (* 2 D_prime))
        ...
        (let ((z_prime (ite (<= (- y_prime 5) C)
          ...
          ; Podmínky pro f a součet
          (and 
            (f D_prime E_prime z_prime)
            (< (+ D_prime E_prime) (+ D E))
          )
        )
      )
